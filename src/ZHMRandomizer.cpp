...
void ZHMRandomizer::OnEngineInitialized() {
    ...
    const ZMemberDelegate<ZHMRandomizer, void(const SGameUpdateEvent&)> s_Delegate(this, &ZHMRandomizer::OnFrameUpdate);
    Globals::GameLoopManager->RegisterFrameUpdate(s_Delegate, 1, EUpdateMode::eUpdatePlayMode);

    // Install a hook to print the name of the scene every time the game loads a new one.
    Hooks::ZEntitySceneContext_LoadScene->AddDetour(this, &ZHMRandomizer::OnLoadScene);
}

ZHMRandomizer::~ZHMRandomizer() {
    // Unregister our frame update function when the mod unloads.
    const ZMemberDelegate<ZHMRandomizer, void(const SGameUpdateEvent&)> s_Delegate(this, &ZHMRandomizer::OnFrameUpdate);
    Globals::GameLoopManager->UnregisterFrameUpdate(s_Delegate, 1, EUpdateMode::eUpdatePlayMode);
}

void ZHMRandomizer::OnDrawMenu() {
...
void ZHMRandomizer::OnDrawUI(bool p_HasFocus) {
    if (m_ShowMessage) {
        ...
        if (ImGui::Begin("ZHMRandomizer", &m_ShowMessage)) {
            // Only show these when the window is expanded.
            ImGui::Text("Hello from ZHMRandomizer!");
        }
        ImGui::End();
    }
}

void ZHMRandomizer::OnFrameUpdate(const SGameUpdateEvent &p_UpdateEvent) {
    // This function is called every frame while the game is in play mode.
}

DEFINE_PLUGIN_DETOUR(ZHMRandomizer, void, OnLoadScene, ZEntitySceneContext* th, ZSceneData& p_SceneData) {
    Logger::Debug("Loading scene: {}", p_SceneData.m_sceneName);
    return HookResult<void>(HookAction::Continue());
}

DECLARE_ZHM_PLUGIN(ZHMRandomizer);
